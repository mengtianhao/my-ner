...
class MyTransformer(nn.Module):
    def __init__(self, d_model=512, nhead=8, 
        ...
        encoder_norm = nn.LayerNorm(d_model) # 层归一化
        ...
        decoder_norm = nn.LayerNorm(d_model) # 层归一化
...
class MyTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        ...
        :param dropout:         丢弃率，论文中的默认值为 0.1    
        """
        # 对于Encoder部分来说，其内部主要由两部分网络所构成:多头注意力机制和两层前馈神经网络
        # 1.先进行多头注意力计算得到z和权重w
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        :return:
        """
        # 计算多头注意力，输入为q,k,v,返回z
        src2 = self.self_attn(src, src, src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask, )[0]
        # src2: [src_len,batch_size,num_heads*kdim]
        src = src + self.dropout1(src2)  # 残差连接
        src = self.norm1(src)  # [src_len,batch_size,num_heads*kdim]
        # 两层前馈神经网络，其中在第一层后加入了relu激活函数
...
class MyTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None,
        ...
        tgt = self.norm1(tgt)  # [tgt_len,batch_size, embed_dim]，层归一化
        # 2.解码部分输入序列之间'的多头注意力（也就是论文结构图中的Masked Multi-head attention)
        # 与解码部分的输入进行交互，q来自解码部分，k，v来自memory
        tgt2 = self.multihead_attn(tgt, memory, memory,  attn_mask=memory_mask, key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout2(tgt2)  # 残差连接
        tgt = self.norm2(tgt)  # [tgt_len, batch_size, embed_dim]，层归一化
        # 同Encoder部分的两层前馈神经网络
...
def multi_head_attention_forward(query,  # [tgt_len,batch_size, embed_dim]
                                ...
                                q_proj=None,  # [embed_dim,kdim * num_heads]，kdim * num_heads = d_model
    ...
    scaling = float(head_dim) ** -0.5 # q的放缩因子，
    q = q * scaling  # [query_len,batch_size,kdim * num_heads]，q被放缩
    ...
    # contiguous()方法是将变量放到一块连续的物理内存中
    # q:[tgt_len,batch_size,kdim * num_heads]
    ...
    # 加入残差连接
    # F.dropout 是一个函数，参数包含输入的tensor，概率和training 为真还是假。
    # 当training 是真的时候，才会将一部分元素置为0，其他元素会乘以 scale 1/(1-p).
    # training 为false的时候，dropout不起作用。
    # 默认情况下training是True。
    attn_output_weights = F.dropout(attn_output_weights, p=dropout_p, training=training)
    # torch.bmm 矩阵乘法，将Q和K的结果与V相乘
